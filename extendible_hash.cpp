...
void ExtendibleHash<K, V>::Insert(const K &key, const V &value) {
  //找到要插入的桶
  int idx = getIdx(key);
  //声明指针指向该bucket（shared_ptr）
  shared_ptr<Bucket> cur = buckets[idx];
  while (true) {
    //给该指针上锁
    lock_guard<mutex> lck(cur->latch);
    //找是否有key值相等的，如果有就覆盖，如果没有且位置没超过bucketSize就在最后面加上该值。然后break。（end()是链表后一位）
    ...
    {
      lock_guard<mutex> lck2(latch);
      //如果localDepth>globalDepth就需要扩展buckets，负责就不需要（之前就扩展过）
      ...
      bucketNum++;
      //声明新的桶(make_shared)智能指针
      //make_shared<Bucket>(cur->localDepth)括号内为构造函数的参数
      auto newBuc = make_shared<Bucket>(cur->localDepth);

      typename map<K, V>::iterator it;
      //将原来桶的元素分成两个桶，并将元素分别装进两个桶
